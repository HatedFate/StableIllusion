...
def get_clean_sample(model, sample, model_output, timestep, pred_type="epsilon"):
    alphas_cumprod = model.scheduler.alphas_cumprod.to(model.device) 

    alpha_prod = alphas_cumprod[timestep]                # beta_prod = 1-alpha_prod
    sqrt_alpha_prod = torch.sqrt(alpha_prod)
    sqrt_one_minus_alpha_prod = torch.sqrt(1 - alpha_prod)  # aka sqrt_beta_prod

    if pred_type == "epsilon":
        return (sample - sqrt_one_minus_alpha_prod * model_output) / sqrt_alpha_prod

    elif pred_type == "sample":
        return sample

    elif pred_type == "v_prediction":
        return sqrt_alpha_prod * sample - sqrt_one_minus_alpha_prod * model_output
...
def get_text_embedding(model, prompts):
    ...
    text_input = model.tokenizer(
        ...
        truncation=True, return_tensors='pt').input_ids.to(model.device)
    
    uncond_input = model.tokenizer(
        [model.uncond_text] * len(prompts), 
        padding='max_length', 
        max_length=model.tokenizer.model_max_length, 
        return_tensors='pt').input_ids.to(model.device)

    with torch.no_grad():
        text_embeddings = model.text_encoder(text_input.to(model.device))[0]
        uncond_embeddings = model.text_encoder(uncond_input.to(model.device))[0]
...
@torch.no_grad()
def get_images(model, latents: list):
    images = []
